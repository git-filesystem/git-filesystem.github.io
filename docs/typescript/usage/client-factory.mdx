...
The main entrypoint into the main library is via the `ClientFactory` class.
...
These methods will use pretty-printed JSON using 2 spaces for indentation by default, but you can use this option to change that.
...
type FormatJsonConfig = {
  prettyFormat: true;
  indentChar: " " | "\t";
  indentSize: number; // How many of the `indentChar`
};
```
